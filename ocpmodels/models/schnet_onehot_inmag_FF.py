...
class EmbeddingBlock(torch.nn.Module):
    ...
    def forward(
        ...
        #self.spin_emb = s #encode_to_one_hot(s) #!gradient
...
@registry.register_model("schnet_onehot_inmag_FF")
...
#!wx
    ...
    def forward(self, data):
        ...
        #breakpoint()
        #!wx for computing stress
        if self.regress_stress:
            batch_size = data.cell.shape[0]
            strain = torch.zeros_like(data.cell, requires_grad=True)
            data.cell = data.cell + torch.bmm(data.cell, strain)
            cross_product = torch.linalg.cross(data.cell[:, 1], data.cell[:, 2])
            volumes = torch.einsum('bi,bi->b', data.cell[:, 0], cross_product)
        ...
        if self.regress_forces and self.regress_stress == False :
            forces = -1 * (
                torch.autograd.grad(
                    energy,
                    data.pos,
        ...
        elif self.regress_forces and self.regress_stress:
            forces = -1 * (
                torch.autograd.grad(
                    energy,
                    data.pos,
            ...
            stress = torch.autograd.grad(energy, 
                    strain, 
                    grad_outputs=torch.ones_like(energy), 
                    create_graph=True,
                    retain_graph=True,
            ...
            return energy, _M, forces, stress
