...
@registry.register_model("schnet_Sopt")
...
#!wx
    ...
    def _forward(self, data):
        ...
        if self.use_pbc:
            ...
            h = self.embedding(z) #!self.embedding = Embedding(100, hidden_channels, padding_idx=0) hidden 128
...
class SpinDistanceEdge(torch.nn.Module):
    def __init__(
        ...
        self.lin_spin = nn.Linear(
            3 + num_gaussians, num_gaussians, bias=True
        ) #[50 + 50] to [50]
        ...
        self.layers_gaussian = torch.nn.ModuleList(
            [
                ResidualLayer(3, activation=act)
    ...
    def forward(self, dist, magft_cat, i, j):
        ...
        _spin_edge = encode_to_one_hot((magft_cat[i] * magft_cat[j]).sum(dim=1, keepdim=True).long()).to(torch.float)
