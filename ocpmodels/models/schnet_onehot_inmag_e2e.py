...
def map_to_vector(x):
    ...
    targets = torch.tensor([[0, 0, 0], [0, 0, 1], [0, 0, -1]], dtype=torch.float32).to(device)

    # Choose the index with the highest score from softmax
    _, indices = torch.max(x, 1)

    # Map the indices to the corresponding target vectors
...
@registry.register_model("schnet_onehot_inmag_e2e")
...
#!wx
        ...
        #self.fc2 = nn.Linear(64, 48) #!two layers
        self.fc3 = nn.Linear(64, 3)  # Three outpu
    ...
    def _forward(self, data):
        ...
        # if self.train_e2e:
        # magft_cat = F.silu(self.fc1(data.y.float().unsqueeze(1)))
        # #magft_cat = F.silu(self.fc2(magft_cat)) #!two layers
        # magft_cat = F.softmax(self.fc3(magft_cat)) 

        magft_cat = F.softmax(data.y.float().unsqueeze(1)) 
        magft_cat = map_to_vector(magft_cat)
        # else:
        ...
        if self.use_pbc:
            ...
            #breakpoint()
            h = self.combined_emb(z, magft_cat[:, -1].long(), data.inmag) #!replace mgft_cat
