...
from .layers.embedding_block import AtomEmbedding, EdgeEmbedding, SpinDistanceEdge
...
@registry.register_model("gemnet_oc_SEGNN")
...
#ANCHOR embedding
        ...
        self._spinedge = SpinDistanceEdge(
            ...
            segnn_gaussians=segnn_gaussians
        )
...
#ANCHOR forward
        ...
        magft_cat = magft_cat.to(main_graph["edge_index"].device) #! check 

        # Embedding block
        h = self.atom_emb(atomic_numbers)
        # (nAtoms, emb_size_atom)
        # m = self.edge_emb(h, basis_rad_raw, main_graph["edge_index"])
        #                  #!h, distance_edge, spin_edge, edge_index)

        #!now it dosen't change dim of message because of dense layer.
        m = self._spinedge(h, basis_rad_raw, magft_cat, main_graph["edge_index"])
