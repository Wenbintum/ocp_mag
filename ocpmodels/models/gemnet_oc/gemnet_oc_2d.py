...
@registry.register_model("gemnet_oc_2d")
class GemNetOC(BaseModel):
    ...
    def forward(self, data):
        ...
        x_E = self.out_mlp_E(torch.cat(xs_E, dim=-1))  #!wx last embedding operation
        ...
        with torch.cuda.amp.autocast(False):
            output_tensor = self.out_energy(x_E.float()) #!wx
            E_t, M_t = output_tensor[:, 0].unsqueeze(1), output_tensor[:, 1].unsqueeze(1)
            if self.direct_forces:
