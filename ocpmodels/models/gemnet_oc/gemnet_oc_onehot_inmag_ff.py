...
from e3nn import o3
from torch_geometric.nn.models.dimenet import ResidualLayer as DResidualLayer
...
from .layers.radial_basis import GaussianBasis
...
class EmbeddingBlock(torch.nn.Module):
    def __init__(
        ...
        self.spin_fc = Linear(3 + inmag_channels, 
                              spin_hidden_channels + inmag_channels)
    ...
    def forward(
        self,
        x: Tensor,
        s: Tensor,  # rbf: Tensor, i: Tensor, j: Tensor
        in_mag: Tensor,
    ) -> Tensor:
        ...
        # import pdb
        # pdb.set_trace()
        self.spin_emb = s #encode_to_one_hot(s)
...
class SpinDistanceEdge(torch.nn.Module):
    def __init__(
        ...
        self.layers_rbf = torch.nn.ModuleList(
            [DResidualLayer(num_rbf, act) for _ in range(num_layers_rbf)]
        )

        self.layers_gaussian = torch.nn.ModuleList(
            [
                DResidualLayer(num_gaussians, act)
...
@registry.register_model("gemnet_oc_onehot_inmag_ff")
class GemNetOC(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,  # backwards compatibility with deprecated arguments
        ...
        )  # YURI: Replace atom_emb and edge_emb
        ...
        self.out_mlp_M = torch.nn.Sequential(*out_mlp_M)
        self.out_energy = Dense(
            emb_size_atom, num_targets, bias=False, activation=None
        )
        self.out_M = Dense(emb_size_atom, 1, bias=False, activation=None)
    ...
    def forward(self, data):
        ...
        i, j = main_graph["edge_index"] #!this gonna be the new generated edge
        # magft_cat = magft_cat.to(main_graph["distance"].device)
        # rbf = self.rbf(main_graph["distance"], magft_cat, i, j)  # spin_edge

        # Embedding block
        # h = self.atom_emb(atomic_numbers)
        h = self.emb(
            data.atomic_numbers.long() - 1,
            data.magft,  # rbf, i, j
            data.inmag, #!initial magmom
        )  # atom emb with mag ft
        # convert h to node level

        # (nAtoms, emb_size_atom)
        m = self.edge_emb(
            h, basis_rad_raw, main_graph["edge_index"]
        )  # YURI: Replaced by spin_edge_emb?
        # m = rbf
        ...
        x_M = self.out_mlp_M(torch.cat(xs_E, dim=-1))
        ...
        # else:
        #     E_t = E_t.squeeze(1)  # (num_molecules)
        #     return E_t, M_pred # YURI: for joint loss predictions
        
        if self.regress_stress:
            if self.direct_stress:
                if self.outer_product_stress:
                    S_t_scalar, S_t_irrep2 = self.stress_head(main_graph["vector"], S_st, idx_t, data) #self.stress_head(V_st, F_st, edge_index, data)
                
                elif self.decomposition_stress:
                    S_t_scalar, S_t_irrep2 = self.stress_head(S_st, main_graph["vector"], idx_t, data)
            else:
                #TODO write down the implementation of Nequip with a similar file as calc_forces_and_update to avoid nans and 0
                stress = -torch.autograd.grad(E_t * data.natoms, 
                                              grad_stress, 
                ...
                S_t = stress / volume # + or - ?
                S_t *= 1602.176565 # change the unit from V / A^3 to kBa 
...
# This is forked from the stress_trainer branch
# https://github.com/Open-Catalyst-Project/ocp/blob/ebec229af31b0710bd924c8d40ccbb1dc8622ece/ocpmodels/models/utils/outer_block.py#
class Rank2Block(nn.Module):
    ...
    def forward(self, edge_distance_vec, x_edge, edge_index, data):
        ...
        if self.edge_level:
            ...
            node_outer = scatter_det(edge_outer, edge_index, dim=0, reduce="mean")
        else:
        ...
        if self.extensive:
            stress = scatter_det(node_outer, data.batch, dim=0, reduce="sum")
        else:
            stress = scatter_det(node_outer, data.batch, dim=0, reduce="mean")
        return stress
